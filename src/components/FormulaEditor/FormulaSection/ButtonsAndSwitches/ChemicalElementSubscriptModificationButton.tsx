...
import { Colors, dracGreen, dracRed } from "@/theme/colors/colors"
...
> = ({ isUpArrow, chunkIndex, elementIndex }) => {
	...
	if (isUpArrow === true) {
		return (
			...
			>
				<TiArrowUpOutline color={Colors.fg.lighter.value} />
			</Button>
		)
	} else {
		return (
			...
			>
				<TiArrowDownOutline color={Colors.fg.lighter.value} />
			</Button>
		)
	}
}
